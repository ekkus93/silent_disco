...
- [x] Record the current physical line count. **887 lines** (`rust/silent-disco-core/src/transport/socket/host.rs`).
- [x] Run RUST-CORE, ANDROID, and DESKTOP before editing and record the baseline result. RUST-CORE: `cargo fmt --all -- --check` clean; `cargo clippy --workspace --all-targets --all-features -- -D warnings` clean; `cargo test --workspace --all-features` — 298 passed, 0 failed, 1 ignored (lib), plus every integration-test binary green. ANDROID: `./gradlew assembleDebug test lintDebug --stacktrace --console=plain` — BUILD SUCCESSFUL in 3m 26s, 112 actionable tasks. DESKTOP: `npm run check` was run post-split only (fresh worktree needed `npm install`/`npm run build` for `dist/` first); see 7.4 for its clean result.
- [x] Confirm the responsibility clusters found during investigation: datagram send-timeout policy (`DATAGRAM_SEND_TIMEOUT`, `is_datagram_send_timeout`, and its small inline `#[cfg(test)] mod send_timeout_classification_tests`), peer/route data model (`PeerState`, `PeerRoute`), socket bind/setup (`SocketHostTransport` struct + `bind`), peer lookup/delivery accounting (`pending_peer_for_device`, `peer_for_device`, `authorized_routes`, `record_peer_result`), audio/sync broadcast fan-out (`broadcast_datagram`), peer authorization (`authorize_peer_with_routes`), the public `impl HostTransportNode for SocketHostTransport` trait surface (12 methods), and `impl Drop`. Confirmed exactly as investigated; no additional clusters found.
- [x] Confirm the one real intra-crate coupling: `rust/silent-disco-core/src/transport/socket/host_workers.rs` imports `PeerState`/`PeerRoute` directly (`use super::host::{PeerRoute, PeerState};`) and constructs/mutates them — both are already `pub(super)` and must stay reachable at the same relative path after the split. Confirmed by full re-read of `host_workers.rs`: it also directly reads/writes essentially every `PeerState`/`PeerRoute` field and calls `.transport_peer()`/`.device_id()`/`.mark_inbound_activity()`/`.close()` on `PeerState` values, so both types, all their fields, and those four methods needed visibility reaching `crate::transport::socket` (one level broader than the new `host::peer` definition site), not just `host` — resolved with `pub(in crate::transport::socket)` at the new deeper definition site, mirroring the Task 3 `pub(in crate::platform)` precedent recorded in `memory.md`.
- [x] Confirm `transport/tests.rs` and `types.rs` reference `SocketHostTransport` only in doc comments and exercise it exclusively through the `HostTransportNode` trait object via `SocketTransportFactory::bind_host(...)` — no direct method-name coupling to preserve there. Confirmed; `transport/tests.rs`'s `host_transport_*` tests all still pass unmodified (see 7.4).
- [x] Note the sibling layout of `transport/socket/`: `mod.rs`, `host.rs` (this file), `host_workers.rs` (574 lines), `listener.rs` (655 lines), `shared.rs` (503 lines) — none are directories yet; this task only converts `host.rs`, leaving the others flat (a future task if `listener.rs` also needs it). Confirmed; only `host.rs` was converted, the other three siblings are untouched.

## 7.2 Module design

- [x] Convert `host.rs` into a `host/` directory (`socket/mod.rs`'s `mod host;` resolves to `host/mod.rs` automatically, no change needed there — confirmed: `socket/mod.rs` required zero edits):
...
- [x] Note the Rust constraint driving this shape: `impl HostTransportNode for SocketHostTransport` cannot itself be split across files (one trait impl per type) — but at ~225 lines estimated, it does not need to be. Landed at 237 lines (`node.rs`), close to the estimate.
- [x] Everything else is an inherent `impl SocketHostTransport { ... }` block or a free item, which Rust allows spread across as many files as needed within the module — follow the `pub(super)` convention already established between `host.rs` and `host_workers.rs`. Used `pub(super)` (reaching the `host` subtree) for cross-submodule inherent methods (`pending_peer_for_device`, `peer_for_device`, `authorized_routes`, `record_peer_result`, `broadcast_datagram`, `authorize_peer_with_routes`, `PeerState::is_inbound_silent`), and `pub(in crate::transport::socket)` (reaching `host_workers.rs`, one level broader) for `PeerState`/`PeerRoute` themselves, their fields, and the four methods `host_workers.rs` calls directly. `SocketHostTransport`'s own struct fields stayed fully private (no modifier) since nothing outside the `host` subtree touches them — private items in Rust are automatically visible to the defining module's descendants, so every `host/*.rs` submodule already sees them with no extra annotation.

## 7.3 Behavioral preservation

- [x] Preserve `DATAGRAM_SEND_TIMEOUT` (5ms) and the reasoning behind it: sockets stay blocking (never non-blocking, which would also make reads non-blocking via the shared FD); a send that can't finish in one packet period must fail fast as a *counted, visible* delivery failure. Const + its full doc comment moved verbatim into `bind.rs`, the only file that reads it (`set_write_timeout` calls); no longer needs cross-file visibility so it stayed a plain private `const`.
- [x] Preserve a send timeout **not** counting toward `consecutive_failures`/auto-disconnect — only genuine I/O errors do. This is a confirmed-on-hardware distinction; collapsing it would disconnect a still-live listener on routine congestion. Preserved verbatim in `broadcast.rs`'s `broadcast_datagram` (the `if is_datagram_send_timeout(&error) { continue; }` branch, with its full explanatory comment, still skips `record_peer_result` while still counting the delivery failure).
- [x] Preserve silent-peer eviction via `PeerState::last_inbound_millis`/`authorized_routes` reusing the same `PeerState::close` path that `record_peer_result`'s failure-threshold eviction uses, so both surface through the identical `PeerDisconnected` event. Preserved: `authorized_routes` (`lookup.rs`) still calls `route.peer.close()` on silence timeout; `record_peer_result` (`lookup.rs`) still calls `peer.close()` on the failure threshold; both reuse the single `PeerState::close` (`peer.rs`).
- [x] Preserve the per-listener anti-spoofing authorization check in `authorize_peer_with_routes` (route IPs must equal the authenticated control peer's own remote IP) and the one-authorization-per-device-id rule. Preserved verbatim in `authorization.rs` (the `routes.synchronization.ip() != peer.remote.ip() || routes.audio.ip() != peer.remote.ip()` check and the `route_registry.contains_key(device_id)` check).
- [x] Preserve the protocol/session guards in `broadcast_datagram`/`send_pending_control`/`send_control`/`broadcast_control` (frame kind must match channel; every outbound frame's `session_id` must match `self.session_id`). Preserved verbatim: the channel/frame-kind `match` in `broadcast_datagram` (`broadcast.rs`) and the four `message.session_id() != &self.session_id` / `frame.session_id() != &self.session_id` guards in `send_pending_control`/`send_control`/`broadcast_control` (`node.rs`) and `broadcast_datagram` (`broadcast.rs`) are unchanged.
- [x] Preserve `is_datagram_send_timeout` staying a pure, independently unit-testable function (the reason it was extracted in the first place — the full path is only reproducible on real hardware). Preserved as a private free function in `broadcast.rs`, with its `#[cfg(test)] mod send_timeout_classification_tests` kept nested inline in the same file exactly as before.
- [x] Preserve idempotent `shutdown()` (guards on `shutdown_complete`; closes all peers and joins all workers even if the peer registry lock is poisoned, recording that as an error rather than panicking). Preserved verbatim in `node.rs`'s `impl HostTransportNode`; `impl Drop for SocketHostTransport` (in `mod.rs`) still calls it via `drop(self.shutdown())`.

## 7.4 Acceptance criteria

- [x] `silent_disco_core::transport::SocketHostTransport` resolves unchanged. `transport/socket/mod.rs`'s `pub use host::SocketHostTransport;` and `transport/mod.rs`'s `pub use socket::{SocketHostTransport, ...};` both needed zero edits; confirmed by every downstream build (RUST-CORE, ANDROID, DESKTOP) succeeding unchanged.
- [x] `host_workers.rs` compiles unchanged against `super::host::{PeerRoute, PeerState}` (or an equivalent re-export path). `host_workers.rs` was not edited at all; `host/mod.rs`'s `pub(super) use peer::{PeerRoute, PeerState};` re-export satisfies its existing `use super::host::{PeerRoute, PeerState};` import verbatim.
- [x] `transport/tests.rs`'s `HostTransportNode`-trait-object-based integration tests all still pass. Confirmed: `host_transport_completes_join_request_through_approval_and_disconnect`, `host_transport_delivers_join_rejection`, `host_transport_authorizes_listener_and_exchanges_sync_and_audio` all pass.
- [x] The `send_timeout_classification_tests` module still passes (moved or kept inline in `broadcast.rs`). Confirmed: `transport::socket::host::broadcast::send_timeout_classification_tests::{would_block_is_classified_as_a_send_timeout, other_error_kinds_are_not_classified_as_a_send_timeout}` both pass.
- [x] RUST-CORE, ANDROID, and DESKTOP all pass. See the real command results recorded in `memory.md`'s Task 7 entry.
- [x] No file in the split exceeds 800 lines. Largest is `bind.rs` at 241 lines.
- [x] Record final line counts and the final file list. `rust/silent-disco-core/src/transport/socket/host/`: `mod.rs` 49, `peer.rs` 80, `bind.rs` 241, `lookup.rs` 113, `broadcast.rs` 166, `authorization.rs` 87, `node.rs` 237 — **973 lines across 7 files**, replacing the original 887-line flat `host.rs` (removed).
- [x] Commit only Task 7 changes with a focused commit message.
