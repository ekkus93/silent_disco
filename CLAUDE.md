...
This repository is an **Android-only proof of concept** for an **offline silent disco app**.

The goal is to validate whether:
...
This is a **viability-focused PoC**, not a production app.

## Confirmed project decisions

- Use **Kotlin** for the Android app.
- Use **Jetpack Compose** for UI.
- Use **Oboe** for the playback path.
- **BLE is included in phase 1**, but only for discovery/metadata assistance.
- **Wi-Fi Direct is the primary transport** for session/data flow.
- Keep framing/serialization **simple initially**.
...
Organize code by feature/domain, not by generic technical layer alone. Favor clear boundaries around:
...
Model host and listener behavior explicitly with state machines or strongly typed state models.
...
- Initial audio baseline:
  - PCM
  - 16-bit
  - stereo
  - 48 kHz
- Initial packet duration target: **20 ms**
- Initial startup buffer target: **roughly 300–500 ms**
...
- Do not hide transport, sync, or playback failures behind generic success states.

## Out of scope for this phase

Unless the user explicitly changes scope, do not optimize for:
- iOS support
...
- Prefer simple, readable implementations that help validate the PoC quickly.
- Preserve the distinction between **discovery**, **authorization**, **transport**, **sync**, and **playback**.
- Do not trade away timing visibility for abstraction neatness.
- Keep sync-sensitive paths explicit and instrumentable.
- If a design choice conflicts with low-latency, deterministic playback, favor the option that better supports measurement and correction.
- When unsure, optimize for **sync correctness and observability** over convenience.

## Ralph Loop workflow

This project uses a **Ralph Loop** pattern for iterative implementation:

1. Pick the next unchecked `[ ]` item from `docs/TODO.md`
2. Implement it
3. Write unit tests for it
4. Run the tests:
   - **Pass** → commit to git, push to GitHub, mark the item `[x]` in `docs/TODO.md`, move to the next item
   - **Fail** → keep iterating on the current item until tests pass, then commit and push
5. Repeat

Each git commit should correspond to one checked-off TODO item, with passing tests as the gate. The `memory.md` file provides context across fresh sessions so each loop iteration can pick up where the last left off.
