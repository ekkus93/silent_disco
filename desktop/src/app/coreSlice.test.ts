...
describe("authoritative core slice", () => {
  ...
  it("accepts a newer revision and replaces the complete snapshot", () => {
    let state = coreReducer(
      undefined,
      coreActions.bridgeReady({ profileId: "main", snapshot: snapshot("4", "idle") }),
    );
    state = coreReducer(
      state,
  ...
  it("rejects an equal revision and increments the stale counter", () => {
    let state = coreReducer(
      undefined,
      coreActions.bridgeReady({ profileId: "main", snapshot: snapshot("8") }),
    );
    state = coreReducer(
      state,
      coreActions.notificationReceived({
  ...
  it("rejects an older revision without regressing lifecycle state", () => {
    let state = coreReducer(
      ...
      coreActions.bridgeReady({
        profileId: "main",
        snapshot: snapshot("10", "streaming"),
      }),
    );
    state = coreReducer(
      state,
  ...
  it("records command failure visibly and removes the pending command", () => {
    ...
    state = coreReducer(
      state,
      coreActions.commandFailureObserved({ operationId: "operation-2", error: commandError }),
    );
