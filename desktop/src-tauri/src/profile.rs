...
impl ProfileId {
    ...
    pub fn parse(value: &str) -> Result<Self, ProfileValidationError> {
        ...
        let first = characters.next().ok_or(ProfileValidationError::InvalidId(
            "profile ID must not be blank",
        ))?;
        let last = value
            .chars()
...
impl ProfileMetadata {
    ...
    #[must_use]
    pub const fn new(profile_id: ProfileId, display_name: ProfileDisplayName) -> Self {
        Self {
    ...
    /// # Errors
    ///
    /// Returns [`ProfileValidationError`] when the metadata is from a newer
    /// schema or names a different profile.
...
pub enum ProfileValidationError {
    ...
    /// Metadata is newer than this build understands.
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedMetadataVersion { found, supported } => write!(
                formatter,
                "profile metadata schema {found} is newer than supported schema {supported}"
            ),
...
mod tests {
    ...
    fn rejects_unsafe_or_ambiguous_profile_ids() {
        for value in ["", " Main", "main/other", "..", "UPPER", "-main", "main-"] {
            assert!(ProfileId::parse(value).is_err(), "unexpectedly accepted {value:?}");
        }
    ...
    #[test]
    fn metadata_rejects_newer_schema_and_wrong_profile() {
